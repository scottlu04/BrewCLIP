...
class DataLoader:
  ...
  def __init__(self, local_root_dir: str, dataset_and_split : str):
    ...
    lines = []
    print('(down)loading localized narratives metadata to memory')
    self.download_annotations(dataset_and_split)
    for local_file in self._find_files(dataset_and_split):
      current_open_file = open(local_file, 'rb')
      for line in current_open_file:
        lines.append(line)
    self.lines = lines
    
  def __getitem__(self, idx):
    return LocalizedNarrative(**json.loads(self.lines[idx]))
  ...
  def _download_one_audioclip(self, filename : str):
    if not os.path.exists(self._local_file(filename)):
      wget.download(f'{_RECORDINGS_ROOT_URL}/{filename}',
                    self._local_file(filename), bar=None)
  def get_audioclip(self, filename : str):
    self._download_one_audioclip(filename)
    filepath = self._local_file(filename)
    return filepath