...
def retrieval(ids, images, texts):
    all_ids = torch.LongTensor(ids)
    #all_ids = torch.cat([x["id"] for x in outputs], dim=0)
    feat_dict = []
    image_feat = torch.LongTensor(images)
    print(image_feat.size())
    id_feat_pairs = {_id.item(): _x for _id, _x in zip(all_ids, image_feat)}
    image_feats = torch.stack([x for _, x in id_feat_pairs.items()], dim=0)
    image_feats_id = torch.LongTensor(list(id_feat_pairs.keys()))
    
    print("Total #{} {}".format(len(image_feat),'image'))
    feat_dict.append(['image',image_feats, image_feats_id])
    text_feat = torch.LongTensor(texts)
    feat_dict.append(['aud',text_feat, all_ids])
    print("Total #{} {}".format(len(text_feat),'audio'))
    ...
    all_C_feats = None
    recall_results_AB,recall_results_BA = Bi_Retrieval(all_A_feats,all_B_feats,all_A_feats_id,all_B_feats_id,feat_A_name,feat_B_name)
