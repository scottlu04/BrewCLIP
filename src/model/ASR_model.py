        ...
        return torch.nn.Parameter(
            torch.randn(
                [
                    1,
                    1,
                    self.config.model_settings.transformer_args.d_model,
                ]
            )
        )
...
class ASR_model(Base):
    ...
    def __init__(self, cfg):
        ...
        if 'audio' in modality_list:
            self.asr_type = self.cfg.ASR.type
            if self.asr_type == "Whisper":
                self.ASR = asr(name = self.cfg.ASR.name,)
    ...
    def forward(
        ...
        self.ASR.update_device(self.device)
        image_feat = None
        text_feat = None
        audio_feat = None
    ...
    def getTrainableParams(self) -> list:
        ...
        # if hasattr(self, "img_proj_net"):
        #     my_params += self.img_proj_net.trainable_params()
