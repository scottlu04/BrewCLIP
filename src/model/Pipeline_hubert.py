    ...
    def forward(
       ...
       # wav_len = batch["wav_len"]
        ...
        if self.cfg.clip.prompt:
            transcription_feat, shallow_prompt, deep_prompt, transcription_output  = self.clip.encode_text(transcription)
        ...
        if self.bifurcated:
            ...
            if self.audio_proj_net is not None:
                audio_len = 0
                transcription_feat = self.audio_proj_net(combined_feat,audio_len)
                # print(audio_feat.size())
