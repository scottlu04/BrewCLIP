...
class Base(pl.LightningModule):
    ...
    def training_step(self, batch: dict) -> dict:
        ...
        return {"loss": losses}#{"loss_feats": losses, "log_metrics": log_metrics}
    ...
    def validation_step(self, batch: dict, batch_idx: int) -> dict:
        ...
        return {"loss_feats": out_per_batch} #out_per_batch#{"loss_feats": losses, "log_metrics": log_metrics, "others": return_dict}
    ...
    def reportRetrieval(
        ...
        logging.info(f"val_recall_{log_AB_abbr}", recall_results_AB)
        logging.info(f"val_recall_{log_BA_abbr}", recall_results_BA)
        logging.info("val_recall_mean", recall_results_mean)
        ...
        if self.logger is not None:
            ...
            # )
            self.log(
                "val_recall_mean_1", recall_results_mean["recall@1"], sync_dist=True
            )
        return recall_results_AB,recall_results_BA
    ...
    def forward_audio(
        ...
        if self.audio_encoder_type in [
            ...
            return self.audio_encoder(
                wav, wav_len,prompt, return_hidden_states=return_hidden_states
            )
        else:
