...
class cross_attension_net(nn.Module):
    def __init__(self,cfg) -> None:
        ...
        self.aud_encoder = TransformerEncoder(embed_dim=768)
    ...
    def forward(
       ...
       # wav_len = batch["wav_len"]
        ...
        if self.cfg.clip.prompt:
            transcription_feat, shallow_prompt, deep_prompt, transcription_output,prompt_embeddings  = self.clip.encode_text(transcription)
            ...
            transcription_feat, prompt_embeddings = self.clip.encode_text(transcription)
        ...
        if self.bifurcated:
            ...
            if self.cfg.model_settings.audio_branch.projection_type == "TransformerEncoder":
                audio_output = self.audio_proj_net(audio_output,0)
                #print(transcription_output.size())
                #print(audio_output.size())
                #combined_feat = torch.cat([transcription_output, audio_output],dim=1)
                #print(combined_feat.size())
            else:    
                audio_feat = audio_output[:, 0, :]
                #print(audio_feat.size())
            #combined_feat = transcription_feat + audio_feat
            #audio_output
                # print(audio_feat.size())
        #image_feat, transcription_feat = self.clip.encode_prompt(image,text)
        #encode_prompt
        audio_feat = transcription_feat + audio_output
