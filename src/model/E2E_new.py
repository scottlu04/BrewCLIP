...
from transformers import AutoTokenizer,AutoProcessor, \
    CLIPTextModel,CLIPVisionModel, CLIPModel,CLIPTextModelWithProjection, CLIPVisionModelWithProjection, \
        ...
        return torch.nn.Parameter(
            torch.randn(
                [
                    1,
                    1,
                    self.cfg.model_settings.transformer_args.d_model, #1024
                ]
            )
        )
...
class e2e(Base):
    ...
    def __init__(self, cfg):
        ...
        if 'image' or 'text' in modality_list:
            ...
            self.text_model = CLIPTextModelWithProjection.from_pretrained(self.cfg.clip.name)
            self.tokenizer = AutoTokenizer.from_pretrained(self.cfg.clip.name)
            self.text_model.eval()

            self.img_model = CLIPVisionModelWithProjection.from_pretrained(self.cfg.clip.name)
            self.processor = AutoProcessor.from_pretrained(self.cfg.clip.name)
            self.img_model.eval()
        ...
        if 'audio' in modality_list:
            ...
            if self.audio_encoder_type == "FairseqHubert":
                ...
                #self.audio_encoder = HubertModel.from_pretrained("facebook/hubert-large-ls960-ft")
        ...
        if 'audio' in modality_list:
            if self.cfg.mode == 'asr':
               ...
               # audio_feat += resi_audio_feat
                ...
                #print(wav.size())
                #audio_feat = self.audio_encoder(wav).last_hidden_state
                #audio_len = audio_feat
                #print(audio_feat.size())
                # print(audio_len)
                # audio_len = torch.tensor([audio_feat.size(dim=0), audio_feat.size(dim=1)])
                # print(audio_len)
