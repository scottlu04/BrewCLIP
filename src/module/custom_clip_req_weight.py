...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts, compound_prompts_deeper_text):
        ...
        outputs, weight = self.transformer(combined)
        ...
        return x, last_hidden_state, weight
...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, clip_model):
        ...
        self.proj = nn.Linear(self.ctx_dim, 1024)
        #self.proj.half()
        self.ctx = nn.Parameter(ctx_vectors)
        # These below parameters related to the shared prompts
        # Define the compound prompts for the deeper layers
        if cfg.shared_audio:
            self.ctx_audio = self.ctx
        # else:
        #     ctx_audio = torch.empty(100, 768, dtype=self.dtype)
        #     nn.init.normal_(ctx_audio, std=0.02)
        #     self.ctx_audio = nn.Parameter(ctx_audio)
        
        # Minimum can be 1, which defaults to shallow MaPLe
        # compound prompts
        self.compound_prompts_text = nn.ParameterList([nn.Parameter(torch.empty(self.n_ctx, 768))
                                                      for _ in range(self.compound_prompts_depth - 1)])
        for single_para in self.compound_prompts_text:
            nn.init.normal_(single_para, std=0.02)
        # Also make corresponding projection layers, for each prompt
        single_layer = nn.Linear(self.ctx_dim, 1024)
    ...
    def get_prompt(self):
        ...
        return self.proj(self.ctx), self.ctx_audio, visual_deep_prompts
    def forward(self,text):
       ...
       # return prompt_embeddings, prompt_tokens,self.ctx_audio, self.proj(self.ctx), self.compound_prompts_text, visual_deep_prompts   # pass here original, as for visual 768 is required
        return prompt_embeddings, prompt_tokens, self.proj(self.ctx), self.compound_prompts_text, visual_deep_prompts,self.ctx_audio  # pass here original, as for visual 768 is required
...
class CustomCLIP(nn.Module):
    ...
    def get_prompt(self):
        return self.prompt_learner.get_prompt()
    def encode_text(self, text):
        if isinstance(text, list):
            ...
            raise TypeError(f"Unknown text type {type(text)}")
  
        if self.prompt:
            #pass 
            prompt_embeddings, prompt_tokens, shared_ctx, deep_compound_prompts_text, deep_compound_prompts_vision,prompt_audio = self.prompt_learner(text)
            #print(prompt_tokens[0])
            text_features,text_last_hidden,text_weight = self.text_encoder(prompt_embeddings, prompt_tokens, deep_compound_prompts_text)
            #print(text_features.size())
           
            #print(deep_compound_prompts_vision.size())

            return text_features, shared_ctx, deep_compound_prompts_vision, text_last_hidden, prompt_embeddings,prompt_audio,text_weight
        else:

            text_features,text_last_hidden =  self.clip_model.encode_text(text)
            #text_features = text_features / text_features.norm(dim=-1, keepdim=True)
            return text_features,text_last_hidden
    def encode_image(self, image, shallow_prompt = None, deep_prompt = None):
        if self.prompt:
            #pass 
            image_features, img_weight = self.image_encoder(image.type(self.dtype), shallow_prompt, deep_prompt)
        else:
            assert shallow_prompt == None, deep_prompt == None
            image_features = self.image_encoder(image.type(self.dtype))
        
        return image_features, img_weight
