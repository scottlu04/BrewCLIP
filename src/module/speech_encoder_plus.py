...
#         filename="decode.log", #os.path.join(log_dir, "decode.log"),
#         filemode="a",
...
def customFunc_hubert_forward_old(
    self,
    source: torch.Tensor,
...
def customFunc_hubert_forward(
    ...
    # mask_indices: (B, T), bool
    if prompt != None:
        audio_ctx = prompt.expand(x.shape[0], -1, -1)#.half()
        x = torch.cat([audio_ctx, x], dim=1)
        ctx_mask = torch.zeros([x.shape[0], prompt.shape[0]])
        ctx_mask = ctx_mask.bool().to(x.device)
        padding_mask = torch.cat([ctx_mask,padding_mask],dim = 1)
        #print(padding_mask)
    x, layer_results = self.encoder(
        x,
        padding_mask=padding_mask,
...
# config = yaml.load(open("../../config/speechCLIP/model_base/spchclp_p.yaml", "r"), Loader=yaml.FullLoader)
# print(config)
# #audio_encoder = FairseqSpeechEncoder_Hubert(**config.audio_encoder)