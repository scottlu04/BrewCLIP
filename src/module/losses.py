...
class MaskedContrastiveLoss(nn.Module):
    ...
    def forward(
        ...
        # Compute logits
        logits = feat_A @ feat_B.t() * temperature + 1e-6
        ...
        exp_logits = torch.exp(logits) * neg_mask_fl
   
        loss = 0
        if self.a2b:
            neg_A2B = torch.log(exp_logits.sum(1) + 1e-6)
            #print(exp_logits)
            loss_A2B = (-pos_logits + neg_A2B).mean()
            loss += loss_A2B
        if self.b2a:
            neg_B2A = torch.log(exp_logits.sum(0) + 1e-6)
...
if __name__ == "__main__": 
    feat_a = torch.rand((32,512))
    feat_b = torch.rand((32,512))
    idx = torch.rand((32))
    loss = MaskedContrastiveLoss()
    loss(feat_a,feat_b,idx)
